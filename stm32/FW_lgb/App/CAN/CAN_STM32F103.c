...
void USB_HP_CAN_TX_IRQHandler (void) {
  ...
  if (CAN->TSR & (1 << 0)) {                      /* request completed mbx 0 */
    CAN->TSR |= (1 << 0);                    /* reset request complete mbx 0 */
    ...
    if (isr_mbx_receive (MBX_tx_ctrl[__CTRL1-1], (void **)&ptrmsg) != OS_R_OK) {
      ...
      CAN->IER &= ~(1 << 0);                       /* disable  TME interrupt */ 
    }
  }
}
...
void USB_LP_CAN_RX0_IRQHandler (void) {
  ...
  if (CAN->RF0R & 3) {                                  /* message pending ? */
    ...
    CAN->RF0R |= (1 << 5);                  /* Release FIFO 0 output mailbox */
  }
}
