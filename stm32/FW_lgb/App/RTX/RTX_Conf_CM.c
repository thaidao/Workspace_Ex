...
//   <i> OS_CLOCK specifies the input clock frequency for the selected timer. 
//   <i> This value is calculated as: f(xtal) / VPBDIV. Example is for 15 MHz at @ 60 MHz CPU clock and VPBDIV = 4 
//   <i> Default: 6000000  (6MHz)
#ifndef OS_CLOCK
 #define OS_CLOCK       72000000	// OsClock = 72MB/4 = 18MB
#endif
...
volatile OS_TID err_task;

	switch(err_task){
	case OS_ERR_STK_OVF:
		/* Identify the task with stack overflow */
		err_task = isr_tsk_get();
		USART_PrChar(USART1,err_task);
		__breakpoint(0);
		break;
	case OS_ERR_FIFO_OVF:
		/* ISR Fifo Queue buffer overflow is detected */
		for (;;);
		break;
	case OS_ERR_MBX_OVF:
		for (;;);
